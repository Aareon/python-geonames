...
def mock_database_exists():
    with patch(
        "geonames.database.database_exists", new_callable=AsyncMock, return_value=True
    ) as mock:
        yield mock
...
def test_search_by_name(runner, mock_db_file, mock_database_exists):
    ...
    with patch(
        "geonames.database.search_by_name",
...
def test_search_database_not_found(runner, mock_config):
    ...
    result = runner.invoke(
        cli, ["search", "--db-file", str(non_existent_db), "--name", "Test"]
    )
    assert result.exit_code != 0
    assert f"Database file not found at {non_existent_db}" in result.output


def test_search_database_tables_not_found(
    runner, mock_db_file, mock_database_not_exists
):
...
def test_import_data(runner, mock_config):
    with patch("geonames.cli.Config", return_value=mock_config), patch(
        "geonames.database.setup_database", new_callable=AsyncMock
    ), patch(
        "geonames.database.get_total_entries", new_callable=AsyncMock, return_value=1000
    ), patch(
        "geonames.database.get_country_count", new_callable=AsyncMock, return_value=50
    ):
        result = runner.invoke(
            cli,
            [
                "import-data",
                "--input-file",
                str(mock_config.TXT_FILE),
                "--db-file",
                str(mock_config.DATABASE_FILEPATH),
            ],
        )

        assert result.exit_code == 0
        assert (
            f"Importing data from {mock_config.TXT_FILE} into {mock_config.DATABASE_FILEPATH}"
            in result.output
        )
...
def test_stats_empty_database(runner, mock_db_file, mock_database_exists):
    with patch(
        "geonames.database.get_total_entries", new_callable=AsyncMock, return_value=0
    ), patch(
        "geonames.database.get_country_count", new_callable=AsyncMock, return_value=0
    ), patch(
        "geonames.database.get_top_countries", new_callable=AsyncMock, return_value=[]
    ):
        result = runner.invoke(cli, ["stats", "--db-file", mock_db_file])
        assert result.exit_code == 0
...
def test_import_data(runner, mock_db_file):
    with patch("geonames.database.setup_database", new_callable=AsyncMock), patch(
        "geonames.database.get_total_entries", new_callable=AsyncMock, return_value=1000
    ), patch(
        "geonames.database.get_country_count", new_callable=AsyncMock, return_value=50
    ):
...
def test_import_data_file_not_found(runner, mock_db_file):
    ...
    with patch("geonames.cli.Path.exists", return_value=False), patch(
        "geonames.cli.check_for_updates", new_callable=AsyncMock, return_value=True
    ), patch("geonames.cli.download_zip", new_callable=AsyncMock), patch(
        "geonames.cli.extract_zip",
        new_callable=AsyncMock,
