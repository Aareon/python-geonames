...
async def zip_server(aiohttp_server):
    async def handle_zip(request):
        ...
        return web.Response(body=zip_buffer.getvalue(), headers={'Content-Type': 'application/zip'})
    ...
    try:
        ...
        with zipfile.ZipFile(test_file, 'r') as zip_ref:
            ...
            with zip_ref.open('test.txt') as zip_file:
                ...
                assert content == expected_content, "File content does not match expected content"
...
async def test_check_for_updates(mocker):
    ...
    with aioresponses() as m:
        ...
        mock_stat.st_mtime = datetime(2015, 10, 21, 7, 29, tzinfo=timezone.utc).timestamp()
        result = await check_for_updates(url, zip_file)
...
async def test_extract_zip(tmp_path):
    ...
    assert extracted_file.read_text() == "Test content", "Extracted content does not match"